...
#### 6-1. 아이템 기반 협업 필터링 : 제품 간의 평점을 바탕으로 코사인 유사도로 구현
#### 6-2. 아이템 기반 협업 필터링의 콜드 스타트 문제가 발생함. 유저들의 구매 데이터가 충분치 않기 때문.
#### 6-3. 신규 고객 혹은 구매 데이터가 적은 고객들에게 제품 추천을 해주기 위해, 리뷰 텍스트를 바탕으로 카테고리 예측 모델 구현
#### 6-4. 고객의 검색어, 문의 내용 등을 활용하여, 해당 고객이 관심이 있는 단어들을 통해 카테고리 예측함. 그리고 유저가 구매하지 않은 제품 중 제품 간의 유사도가 높은 상위 5개 제품을 추천해주는 방식으로 구현
#### 6-5. 카테고리별, 제품 간의 평균 평점에다가 누적 구매 수량을 100으로 나눈 값을 가중치로 더한 새로운 평점을 만듬.
#### 6-6. 카테고리별, 새로운 평점 상위 5개의 제품을 추천해줌
...
### 7. 결과
#### 7-1.앙상블 모델 성능
|카테고리 예측|랜덤포레스트|XGBoost|
|:----------------:|:----------------:|:----------------:|
|Train ACC|0.99|0.83|
|Test ACC|0.72|0.72|
|Test F1|0.71|0.72|

- 사용 모델 : 랜덤포레스트, XGBoost
- 모델링 요소 : Chi2, TF-IDF, GridSearch CV

#### 7-2.딥러닝 모델 성능
|카테고리 예측|LSTM|LSTM(Attention)|LSTM(Glove)|BERT|LSTM(Glove, Attention)|Seq2Seq(LSTM Attention)|
|:----------------:|:----------------:|:----------------:|:----------------:|:----------------:|:----------------:|:----------------:|
|LOSS|0.90|0.81|0.56|0.63|0.86|0.46|
|ACCURACY|0.75|0.76|0.81|0.78|0.71|0.91|
...
![image](https://user-images.githubusercontent.com/76590396/127680264-36925b57-496e-4fa3-b2d6-45d77c1b7d8f.png)
...
## 8. 결론
### 프로젝트 결론
- 신규 고객에게도 고객의 검색 내역 하나만으로도 추천을 해줄 수 있는 시스템을 구현하였다.
